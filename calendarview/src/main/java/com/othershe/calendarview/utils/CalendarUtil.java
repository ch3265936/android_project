...
public class CalendarUtil {
     ...
     * @return
     */
    public static List<DateBean> getMonthDate(int year, int month, Map<String, String> map, Map<String, Boolean> status) {
        ...
        for (int i = 0; i < week; i++) {
            datas.add(initDateBean(lastYear, lastMonth, lastMonthDays - week + 1 + i, 0, map, status));
        }

        for (int i = 0; i < currentMonthDays; i++) {
            datas.add(initDateBean(year, month, i + 1, 1, map, status));
        }

        for (int i = 0; i < 7 * getMonthRows(year, month) - currentMonthDays - week; i++) {
            datas.add(initDateBean(nextYear, nextMonth, i + 1, 2, map, status));
        }

        return datas;
    }

    private static DateBean initDateBean(int year, int month, int day, int type, Map<String, String> map, Map<String, Boolean> status) {
        ...
        if (status == null) {
            ...
            if (status.containsKey(year + "." + month + "." + day)) {
                dateBean.setStatus(true);
            } else {
                dateBean.setStatus(false);
            }
        }
